...
const createProject = asyncHandler(async (req, res) => {
  const userId = req.user.userId || req.user.userId === 0 ? req.user.userId : req.user.userId;
...
const addMember = asyncHandler(async (req, res) => {
  ...
  const project = await projectService.addMember(req.user.userId, req.params.id, userId);
  res.json(new ApiResponse(true, "Member added", { project }));
});

const removeMember = asyncHandler(async (req, res) => {
  const { userId } = req.params;
  const project = await projectService.removeMember(req.user.userId, req.params.id, userId);
